...
local formatters = {
  ...
  mypy = require("efmls-configs.linters.hadolint"),
...
local efm_settings = {
  languages = {
    ...
    dockerfile = { formatters.hadolint },
...
mason_lspconfig.setup_handlers({
  function(server_name)
    lsp_servers[server_name]["capabilities"] = capabilities

    if lsp_servers[server_name]["on_attach"] ~= nil then
      local lsp_on_attach = lsp_servers[server_name]["on_attach"]
      lsp_servers[server_name]["on_attach"] = function(client, buffer)
        lsp_on_attach(client, buffer)
        on_attach(client, buffer)
      end
    else
      lsp_servers[server_name]["on_attach"] = on_attach
    end
    require("lspconfig")[server_name].setup(lsp_servers[server_name])
  end,
})
